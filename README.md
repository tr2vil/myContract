...
```
데이터 입력 → 계약서 생성 → 미리보기/검토 → (임대인 서명) → 이메일 발송 → 임차인 서명 → 완료
```

1. `계약목록` 시트에 계약 데이터 입력
2. 해당 행 선택 → **계약 관리 → 선택한 행 작업 (사이드바)**
3. 사이드바에서:
   - **계약서 생성** → Google Docs 자동 생성
...
웹앱(서명 페이지)을 수정한 경우 **새 배포**가 필요:
```bash
npm run deploy      # 새 버전 배포
# 설정 시트의 web_app_url을 새 배포 URL로 업데이트
```

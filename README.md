# myContract - 상업용 건물 임대 관리 시스템

Google Sheets + Google Docs + Google Apps Script 기반의 임대 계약서 자동 생성, 이메일 발송, 전자서명 수집 시스템.

## 주요 기능

- **계약서 자동 생성**: 스프레드시트 데이터로 Google Docs 임대차계약서를 자동 생성 (임대인 서명 자동 삽입)
- **미리보기**: 생성된 계약서를 Google Docs에서 바로 확인/수정
- **계약요청 이메일 발송**: 임차인에게 계약서 검토 및 서명 요청 이메일 발송 (첨부파일 지원)
- **전자서명**: HTML5 Canvas 기반 웹 서명 (마우스/터치 지원, 모바일 대응)
- **신분증 업로드**: 서명 시 임차인 신분증 사본 필수 업로드 (Drive 저장 + 계약서 첨부)
- **임대인 확인 절차**: 임차인 서명 후 임대인이 확인한 뒤 최종계약서 이메일 발송
- **SMS 문자 복사**: 커스텀 템플릿으로 SMS 문자 생성 및 클립보드 복사
- **만료 임박 알림**: 사이드바에서 계약만료 7일 전 ~ 만료 후 14일 이내 계약 목록 자동 표시 (D-day 배지, 클릭 시 해당 행 이동)
- **상태 관리**: 작성중 → 서명대기 → 서명완료 → 완료 흐름 자동 추적

## 설치 및 설정

### 사전 요구사항

- Node.js 설치
- Google 계정
- clasp CLI: `npm install -g @google/clasp`

### 1단계: clasp 로그인 및 연결

```bash
clasp login
```

### 2단계: 스프레드시트 생성 및 스크립트 연결

1. Google Sheets에서 새 스프레드시트 생성
2. **확장 프로그램 → Apps Script** 클릭
3. Apps Script 에디터에서 **프로젝트 설정(톱니바퀴)** → **스크립트 ID** 복사
4. `.clasp.json`의 `scriptId`를 복사한 ID로 변경

### 3단계: 코드 배포

```bash
npm run push
```

### 4단계: 스프레드시트 초기 설정

1. 스프레드시트를 새로고침(F5)
2. 상단 메뉴에서 **계약 관리 → 시트 초기 설정** 클릭
3. 권한 승인 화면이 나타나면: **고급 → Go to myContract → 허용**
4. `계약목록`, `설정` 시트가 자동 생성됨

### 5단계: Google Drive 폴더 및 템플릿 준비

1. Google Docs에서 새 문서 생성 → `docs/TEMPLATE_SETUP.md`의 템플릿 내용 붙여넣기
2. Google Drive에 폴더 3개 생성: `계약서`, `서명이미지`, `임차인신분증`
3. `설정` 시트에 각 ID 입력:
   - `template_doc_id`: 문서 URL에서 `/d/`와 `/edit` 사이의 문자열
   - `output_folder_id`: 계약서 폴더 URL에서 `/folders/` 뒤의 문자열
   - `signature_folder_id`: 서명이미지 폴더 URL에서 `/folders/` 뒤의 문자열
   - `id_card_folder_id`: 임차인신분증 폴더 URL에서 `/folders/` 뒤의 문자열
4. 이메일 첨부파일이 있으면 `attachment_file_ids`에 Drive 파일 ID 입력 (쉼표로 구분)

### 6단계: 서명 웹앱 배포

1. Apps Script 에디터에서 **배포 → 새 배포**
2. 유형: **웹 앱**
3. 실행 사용자: **나**, 액세스: **모든 사용자**
4. 배포 URL을 `설정` 시트의 `web_app_url`에 입력

## 사용 방법

### 워크플로우

```
데이터 입력 → 계약서 생성 → 미리보기/검토 → 이메일 발송 → 임차인 서명+신분증 → 임대인 확인 → 최종계약서 발송
```

1. `계약목록` 시트에 계약 데이터 입력
2. 해당 행 선택 → **계약 관리 → 선택한 행 작업 (사이드바)**
3. 사이드바에서:
   - **만료 임박 목록** → 상단에 만료 임박 계약 자동 표시 (클릭 시 해당 행 이동)
   - **계약서 생성** → Google Docs 자동 생성 (임대인 서명 자동 삽입)
   - **미리보기** → 생성된 문서를 Google Docs에서 확인
   - **계약요청 이메일 발송** → 임차인에게 검토+서명 링크 이메일 전송 (첨부파일 포함)
   - **SMS 문자 복사** → 이메일 발송 후 SMS 알림용 문자 클립보드 복사
4. 임차인이 이메일 수신 → 계약서 검토 → 서명 링크 클릭 → 서명 + 신분증 업로드
5. 임차인 서명 완료 → 상태: **서명완료** (임대인 확인 대기)
6. 임대인이 서명 및 신분증 확인 후 사이드바에서 **최종계약서 이메일 전송** 클릭
7. 완료 이메일 발송 → 상태: **완료**

### 계약 상태 흐름

| 상태 | 설명 |
|------|------|
| 작성중 | 초기 상태 |
| 서명대기 | 이메일 발송 후 임차인 서명 대기 중 |
| 서명완료 | 임차인 서명 완료, 임대인 확인 대기 |
| 완료 | 임대인 확인 후 최종계약서 발송 완료 |

### 만료 임박 알림

사이드바 상단에 계약 만료가 임박한 항목이 자동 표시됩니다.

| 구분 | D-day 범위 | 배지 색상 |
|------|-----------|----------|
| 만료 초과 | D+1 ~ D+14 | 진한 빨강 |
| 긴급 | D-Day ~ D-3 | 빨강 |
| 주의 | D-4 ~ D-7 | 주황 |

- 만료일 **7일 전**부터 표시 시작
- 만료 후 **14일(2주)**이 지나면 자동으로 목록에서 사라짐
- 항목 클릭 시 해당 행으로 이동하여 바로 작업 가능

### 설정 시트 항목

| 항목 | 설명 |
|------|------|
| template_doc_id | Google Docs 계약서 템플릿 문서 ID |
| output_folder_id | 생성된 계약서 저장 Drive 폴더 ID |
| signature_folder_id | 서명 이미지 저장 Drive 폴더 ID |
| id_card_folder_id | 임차인 신분증 사본 저장 Drive 폴더 ID |
| web_app_url | 서명 웹앱 배포 URL |
| sender_name | 이메일 발신자 이름 |
| sender_email | 발신 이메일 주소 (참고용) |
| company_name | 회사명 (선택) |
| landlord_name | 임대인(갑) 이름 |
| landlord_phone | 임대인 연락처 |
| landlord_email | 임대인 이메일 |
| sms_template | SMS 문자 템플릿 (플레이스홀더 사용 가능) |
| attachment_file_ids | 이메일 첨부파일 Drive 파일 ID (쉼표 구분) |

### Google Docs 템플릿 플레이스홀더

템플릿에서 `{{컬럼헤더}}` 형식으로 작성하면 스프레드시트 데이터로 자동 치환됩니다.

```
{{상호}}    → "상호" 컬럼 값
{{이름}}    → "이름" 컬럼 값
{{보증금}}  → 콤마 포맷 적용 (예: 50,000,000)
{{계약일}}  → "yyyy년 MM월 dd일" 포맷
```

서명란 플레이스홀더:
```
[임대인 서명란] → 계약서 생성 시 임대인 서명 이미지로 자동 대체
[임차인 서명란] → 임차인 전자서명 이미지로 대체
```

### SMS 템플릿 플레이스홀더

`sms_template` 설정에서 사용 가능:

```
{{이름}}, {{지점명}}, {{상호}}, {{층}}, {{호수}}, {{보증금}}, {{공급가액}}
{{서명링크}} → 전자서명 웹앱 URL
{{계약서_링크}} → Google Docs 계약서 URL
```

## 개발

### npm 스크립트

| 명령어 | 설명 |
|--------|------|
| `npm run push` | 코드를 Apps Script에 업로드 |
| `npm run pull` | Apps Script에서 코드 다운로드 |
| `npm run open` | Apps Script 에디터 열기 |
| `npm run open:web` | 배포된 웹앱 열기 |
| `npm run deploy` | 새 배포 생성 |
| `npm run deployments` | 배포 목록 확인 |
| `npm run logs` | 실행 로그 확인 |

### 코드 수정 후 반영

**스프레드시트 기능** (사이드바, 메뉴 등)만 수정한 경우:
```bash
npm run push        # 코드 업로드 (HEAD 업데이트)
# 스프레드시트 새로고침(F5) 후 테스트
```

**웹앱** (서명 페이지, 서명 처리 함수 등)을 수정한 경우:
```bash
npm run push                                          # 코드 업로드
npx clasp deploy -i <deploymentId> -d "변경 설명"      # 기존 배포 업데이트
```

> **중요**: `clasp push`는 HEAD 코드만 업데이트합니다. 웹앱은 **배포된 버전**을 실행하므로, 웹앱 관련 코드를 수정했다면 반드시 `deploy`까지 실행해야 반영됩니다.

## 제한사항

- Google Apps Script 실행 시간 제한: 6분/회
- Gmail 일일 발송 한도: 무료 계정 100통, Workspace 1,500통
- 전자서명은 법적 공인인증이 아닌 간편 서명 방식
- 하나의 스프레드시트에 스크립트가 바인딩됨 (해당 스프레드시트에서만 메뉴 표시)

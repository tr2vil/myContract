...
- **계약서 자동 생성**: 스프레드시트 데이터로 Google Docs 임대차계약서를 자동 생성 (임대인 서명 자동 삽입)
- **미리보기**: 생성된 계약서를 Google Docs에서 바로 확인/수정
- **계약요청 이메일 발송**: 임차인에게 계약서 검토 및 서명 요청 이메일 발송 (첨부파일 지원)
- **전자서명**: HTML5 Canvas 기반 웹 서명 (마우스/터치 지원, 모바일 대응)
- **신분증 업로드**: 서명 시 임차인 신분증 사본 필수 업로드 (Drive 저장 + 계약서 첨부)
- **임대인 확인 절차**: 임차인 서명 후 임대인이 확인한 뒤 최종계약서 이메일 발송
- **SMS 문자 복사**: 커스텀 템플릿으로 SMS 문자 생성 및 클립보드 복사
- **상태 관리**: 작성중 → 서명대기 → 서명완료 → 완료 흐름 자동 추적
...
│   ├── SignatureService.gs  # 토큰 생성, 서명/신분증 이미지 저장/문서 삽입
│   ├── WebApp.gs            # doGet() 서명 웹앱 핸들러
│   └── html/
│       ├── Sidebar.html     # 사이드바 UI (액션 버튼 + 상태 표시)
│       └── SignaturePage.html # Canvas 서명 + 신분증 업로드 페이지 (웹앱)
└── docs/
...
| 시스템 (AD~AM) | 계약서_생성, 문서ID, 링크, 이메일_발송, 발송일, 완료이메일_발송일, 서명_토큰, 임대인_서명, 임차인_서명, 계약_상태 | 시스템 자동 관리 |
...
| id_card_folder_id | 임차인 신분증 사본 저장 Drive 폴더 ID |
...
| sms_template | SMS 문자 템플릿 (플레이스홀더 사용 가능) |
| attachment_file_ids | 이메일 첨부파일 Drive 파일 ID (쉼표 구분) |
...
### 5단계: Google Drive 폴더 및 템플릿 준비

1. Google Docs에서 새 문서 생성 → `docs/TEMPLATE_SETUP.md`의 템플릿 내용 붙여넣기
2. Google Drive에 폴더 3개 생성: `계약서`, `서명이미지`, `임차인신분증`
3. `설정` 시트에 각 ID 입력:
   ...
   - `id_card_folder_id`: 임차인신분증 폴더 URL에서 `/folders/` 뒤의 문자열
4. 이메일 첨부파일이 있으면 `attachment_file_ids`에 Drive 파일 ID 입력 (쉼표로 구분)
...
```
데이터 입력 → 계약서 생성 → 미리보기/검토 → 이메일 발송 → 임차인 서명+신분증 → 임대인 확인 → 최종계약서 발송
```
...
3. 사이드바에서:
   ...
   - **계약요청 이메일 발송** → 임차인에게 검토+서명 링크 이메일 전송 (첨부파일 포함)
   - **SMS 문자 복사** → 이메일 발송 후 SMS 알림용 문자 클립보드 복사
4. 임차인이 이메일 수신 → 계약서 검토 → 서명 링크 클릭 → 서명 + 신분증 업로드
5. 임차인 서명 완료 → 상태: **서명완료** (임대인 확인 대기)
6. 임대인이 서명 및 신분증 확인 후 사이드바에서 **최종계약서 이메일 전송** 클릭
7. 완료 이메일 발송 → 상태: **완료**
...
|------|------|
| 작성중 | 초기 상태 |
| 서명대기 | 이메일 발송 후 임차인 서명 대기 중 |
| 서명완료 | 임차인 서명 완료, 임대인 확인 대기 |
| 완료 | 임대인 확인 후 최종계약서 발송 완료 |
...
[사이드바 UI] → google.script.run → [서버 함수]
                                         ...
                                         ├── sendContractEmail() → Gmail API (첨부파일 포함)
                                         ├── sendCompletionEmail() → Gmail API (최종계약서)
                                         └── processSignature()  → Drive API + Docs API

[서명 웹앱] → doGet(e) → 토큰 검증 → SignaturePage.html
                                         └── processSignature(token, signatureData, idCardData)
                                             ├── 서명 이미지 → signature_folder
                                             ├── 신분증 이미지 → id_card_folder
                                             └── 계약서 문서에 서명 삽입 + 신분증 첨부
```
...
서명란은 특수 플레이스홀더:
```
[임대인 서명란] → 계약서 생성 시 임대인 서명 이미지로 자동 대체
[임차인 서명란] → 임차인 전자서명 이미지로 대체
```

### SMS 템플릿 플레이스홀더

`sms_template` 설정에서 사용 가능한 플레이스홀더:
